<?php
    ...
    //Sipariş verilerinin çekilmesi
    $OrderFetcher=$conn->query("SELECT * FROM orders WHERE OrderID=$OrderID");

    if($row=$OrderFetcher->fetch_assoc()){
        ...
        //Sipariş durumunun renklendirilmesi
        $OrderStatusColor="";
        if($OrderStatus=="Kargoya Verildi"){

            $OrderStatusColor="green";
        }
    
          else if($OrderStatus=="Hazırlanıyor"){
...
<body>
    ...
    <div id="order-info-div" class="container-fluid pt-5 d-flex justify-content-center align-items-center">
        ...
        <div id="order-cell" class="container w-25 d-flex justify-content-center align-items-center flex-column">
            ...
            <p style="color:<?php echo $OrderStatusColor?>"><?php echo $OrderStatus?></p>
        </div>
    </div>
    ...
    <div id="products-container" class="container-fluid d-flex justify-content-start align-items-start flex-column">
        ...
        <?php
            ...
            for($i=0;$i<count($ProductArray);$i++){
                ...
                //Ürünün fotoğraflarının seçilen renge göre belirlenmesi
                if($Productcolor=="Siyah"){
                    ...
                    echo "<div id='product-div' class='container w-75 p-3 d-flex justfiy-content-start align-items-center flex-row'><div style='background-image:url(./TelefonFoto/$ProductID-2.png)' id='photo' class='container w-25'></div><div id='info-div' class='container w-75 pt-3 px-4 d-flex justify-content-start align-items-start flex-column gap-5'><a href='display-product.php?product_id=$ProductID'><h1 class=''>$Productname($Productcolor)</h1></a><br><p class=''> $ProductPrice TL</p><br><p id='quantity-text'>Adet:$ProductQuantity</p></div></div>";
                }

                else{

                    echo "<div id='product-div' class='container w-75 p-3 d-flex justfiy-content-start align-items-center flex-row'><div style='background-image:url(./TelefonFoto/$ProductID-3.png)' id='photo' class='container w-25'></div><div id='info-div' class='container w-75 pt-3 px-4 d-flex justify-content-start align-items-start flex-column gap-5'><a href='display-product.php?product_id=$ProductID'><h1 class=''>$Productname($Productcolor)</h1></a><br><p class=''> $ProductPrice TL</p><br><p id='quantity-text'>Adet:$ProductQuantity</p></div></div>";
                }
            }
    ...
    <div id="cancel-order-div" class="container-fluid d-flex justify-content-center align-items-center pt-5">
        <?php
        
            //Eğer sipariş iptal edilmemişse iptal butonunu göster

            if($OrderStatus!="İptal Edildi"){

                echo "<a href='cancel-order.php?order_id=<?php echo $OrderID?>'>Siparişi İptal Et</a>";
            }
        ?>
    </div>
</body>
