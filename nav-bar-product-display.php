<?php
    ...
    $Email_Session=$_SESSION['email'] ?? '';//Eğer session boş ise alışverişi sepeti bastırılmayacak


    //URL parametresi olan markayı al ve ona ait olan ürünleri göster
    $Brand=$_GET['brand'];

    $ShoppingCartText="";//Alışveriş sepeti texti

    $Sayfa="nav-bar-product-display.php?search-value-input=$Brand";//Sayfa URL'si
    ...
    //Markanın ürünlerinin çeklimesi(ID,Isim ve fiyat)
    $ProductFetcher=$conn->query("SELECT GROUP_CONCAT(product_id) as product_ids,GROUP_CONCAT(price) as prices,GROUP_CONCAT(product_name) as product_names FROM products WHERE brand='$Brand'");

    if($row=$ProductFetcher->fetch_assoc()){
        $ProductIDArray=explode(",",$row['product_ids']);
        $ProductNameArray=explode(",",$row['product_names']);
        $ProductPriceArray=explode(",",$row['prices']);
    }



    //Eğer kullanıcı giriş yapmışsa kullanıcının sepetini çek
    if($Email_Session!=""){
...
<head>
    ...
    <title><?php echo $Brand ." "."Marka Ürünler"?></title>
    ...
    <link rel='stylesheet' type='text/css' media='screen' href='./style/nav-bar-product-display.css?v=<?php echo time(); ?>'>
...
<body>
    ...
    <?php
        ...
        //Eğer session boş ise bunu bastır
        if ($Email_Session == "") {
            echo '
            ...
            <nav class="navbar navbar-expand-lg navbar-light bg-light">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav mr-auto">
                        ...
                        <li class="nav-item">
                            <form action="search-page.php" class="form-inline my-0 my-lg-0">
                                <input name="search-value-input" class="form-control mr-sm-2" type="search" placeholder="Ara..." aria-label="Search">
                                <button class="btn btn-outline-success my-2 my-sm-0" type="submit">Ara</button>
                            </form>
                        </li>
                    </ul>
